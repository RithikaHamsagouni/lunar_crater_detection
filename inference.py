...
from probability_head import (
    ProbabilityHead,
    extract_crater_instances
)
...
def load_model(
    ...
    print(
        ...
        f"dice={checkpoint.get('val_dice', 0):.4f})"
    )
...
def run_inference_on_image(
    ...
    if not prob_out.binary_mask.any():
        ...
        print("[INFO] No crater pixels detected.")
    ...
    if len(instances) == 0:
        ...
        print("[INFO] No crater instances extracted.")

        return [], image_np

    labeled_mask, _ = ndimage.label(
        prob_out.binary_mask[0, 0]
        .cpu()
        .numpy()
        .astype(np.uint8)
    )
    ...
    for inst in instances:
        ...
        cy = np.clip(
            cy,
            0,
            labeled_mask.shape[0] - 1
        )

        cx = np.clip(
            cx,
            0,
            labeled_mask.shape[1] - 1
        )
...
def main():
    ...
    print(
        f"Detected {len(results)} craters!"
    )
